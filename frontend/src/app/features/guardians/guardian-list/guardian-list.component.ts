    ...
    this.dialog.confirm(
      ...
      if (confirmed) {
        this.guardianService.resetPassword(guardian.id!).subscribe({
          next: (res) => {
            this.dialog.alert(
              `Password reset successfully!<br><br><strong>Temporary Password:</strong> <code class="bg-indigo-500/10 text-indigo-400 px-2 py-1 rounded font-mono font-bold">${res.password}</code><br><br>Notification has been sent to ${guardian.email || guardian.phone_number}.`,
