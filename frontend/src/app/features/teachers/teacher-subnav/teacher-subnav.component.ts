    ...
    <nav class="flex items-center gap-2 overflow-x-auto pb-2 mb-6 custom-scrollbar text-xs font-black uppercase tracking-wider">
      @for (link of navLinks; track link.route) {
        <a [routerLink]="link.route"
           routerLinkActive="bg-teal-600 text-white shadow-md shadow-teal-600/25 border-teal-500"
...
export class TeacherSubnavComponent {
  navLinks = [
    { route: '/teachers/portal', label: 'Gradebook', icon: 'fas fa-graduation-cap', iconColor: 'text-teal-400', exact: true },
    { route: '/teachers/lessons', label: 'Lesson Schemes', icon: 'fas fa-book-open', iconColor: 'text-emerald-400', exact: false },
    { route: '/teachers/seating', label: 'Seating & Roll-Call', icon: 'fas fa-chair', iconColor: 'text-teal-400', exact: false },
    ...
    { route: '/teachers/ai-copilot', label: 'AI Co-Pilot', icon: 'fas fa-robot', iconColor: 'text-teal-400', exact: false },
