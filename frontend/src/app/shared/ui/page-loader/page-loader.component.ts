  ...
  template: `
    <div class="flex flex-col items-center justify-center p-8 md:p-16 text-center animate-fade-in" [ngClass]="{ 'min-h-[50vh]': fullHeight }">
      ...
      <div class="relative flex items-center justify-center w-14 h-14 mb-4">
        <div class="absolute inset-0 rounded-full border-2 border-indigo-500/20 animate-ping opacity-40"></div>
        <div class="w-12 h-12 rounded-full border-2 border-indigo-500/30 border-t-indigo-500 border-r-purple-500 animate-spin"></div>
        <div class="absolute w-2.5 h-2.5 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 shadow-lg shadow-indigo-500/50"></div>
      </div>
