...
func initUseCases(repos *Repositories, infra *Infrastructure, db *gorm.DB, cfg *config.Config) *UseCases {
	...
	return &UseCases{
		...
		Student:        usecase.NewStudentUseCase(repos.Student, repos.Grade, repos.Attendance, repos.Welfare, repos.User, repos.Guardian, infra.SMTP, repos.Fiscal, repos.AcademicPeriod, infra.SMS),
		Academic:       usecase.NewAcademicUseCase(repos.Grade, repos.Attendance, repos.Student, repos.Subject, repos.Fiscal),
		Guardian:       usecase.NewGuardianUseCase(repos.Guardian, repos.Student, repos.Fiscal, repos.User, infra.SMTP, infra.SMS),
		Teacher:        usecase.NewTeacherUseCase(repos.Teacher, repos.User, repos.Class, repos.Subject, infra.SMTP, infra.SMS),
