...
func (s *PDFService) GeneratePupilBill(w io.Writer, tenantName string, tenant *domain.Tenant, student *domain.Student, records []domain.FiscalRecord, config *domain.BillTemplateConfig, termName string, academicYear string) error {
	pdf := gofpdf.New("P", "mm", "A4", "")
	pdf.AddPage()
	...
	s.drawBillStudentInfo(pdf, student, termName, academicYear)
...
func (s *PDFService) GenerateBulkPupilBills(w io.Writer, tenantName string, tenant *domain.Tenant, bills []StudentBillData, config *domain.BillTemplateConfig, termName string, academicYear string) error {
	pdf := gofpdf.New("P", "mm", "A4", "")
	...
	for _, bill := range bills {
		...
		s.drawBillHeader(pdf, tenantName, tenant, config, termName)
		s.drawBillStudentInfo(pdf, bill.Student, termName, academicYear)
